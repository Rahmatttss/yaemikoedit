...
let handler = async (m, {
    ...
    if (args.length >= 1) {
        ...
        text = m.quoted.text
    } else throw m.reply("Input teks atau reply teks yang ingin di cari!")
    ...
    try {
        ...
        try {
            ...
            if (!caption.length) throw m.reply(`Query "${text}" Not Found`)
        await conn.sendFile(m.chat, google_img, "", caption, m)
        } catch (e) {
            try {
...
handler.help = ["google", "google"].map(v => v + " <pencarian>")
